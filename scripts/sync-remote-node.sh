...
BRANCH="${OKX_REMOTE_BRANCH:-$CURRENT_BRANCH}"
...
INSTALL_AUTO_UPDATE="${OKX_REMOTE_INSTALL_AUTO_UPDATE:-0}"
AUTO_UPDATE_INTERVAL_MINUTES="${OKX_REMOTE_AUTO_UPDATE_INTERVAL_MINUTES:-2}"
FIRST_ARG="${1:-}"
DRY_RUN=0
...
usage() {
  cat <<'EOF'
用法：
  scripts/sync-remote-node.sh [--dry-run] [--install-auto-update] [--auto-update-interval-minutes N] [user@host]

行为：
  1. 自动从本地 App 读取远端节点 URL、当前环境和鉴权令牌
  2. 通过 SSH 连到远端节点
  3. 自动拉取当前分支代码、重启远端节点
  4. 可选安装远端 auto-update timer
  5. 自动校验远端健康状态
...
可选环境变量：
  ...
  OKX_REMOTE_BRANCH       远端更新分支，默认当前本地分支
  ...
  OKX_REMOTE_INSTALL_AUTO_UPDATE           设为 1 时，同步后自动安装远端 auto-update timer
  OKX_REMOTE_AUTO_UPDATE_INTERVAL_MINUTES  自动更新轮询分钟数，默认 2
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --auto-update-interval-minutes)
      AUTO_UPDATE_INTERVAL_MINUTES="${2:-2}"
      shift 2
      ;;
    *)
      FIRST_ARG="${1:-}"
...
if [[ "$INSTALL_AUTO_UPDATE" == "1" ]]; then
  echo "远端自动更新: 安装，间隔 ${AUTO_UPDATE_INTERVAL_MINUTES} 分钟"
fi
echo
...
ssh "$REMOTE_SSH_TARGET" bash -s -- \
  ...
  "$BRANCH" \
  "$INSTALL_AUTO_UPDATE" \
  "$AUTO_UPDATE_INTERVAL_MINUTES" <<'REMOTE'
set -euo pipefail
...
REPO_URL="$5"
BRANCH="$6"
INSTALL_AUTO_UPDATE="$7"
AUTO_UPDATE_INTERVAL_MINUTES="$8"
...
if [[ "$INSTALL_AUTO_UPDATE" == "1" ]]; then
  export OKX_REMOTE_AUTO_UPDATE_INTERVAL_MINUTES="$AUTO_UPDATE_INTERVAL_MINUTES"
  bash "$APP_DIR/deploy/install-remote-auto-update.sh" "$APP_DIR"
fi
REMOTE
