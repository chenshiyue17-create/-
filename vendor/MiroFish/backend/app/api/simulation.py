...
def _local_graph_exists(graph_id: str) -> bool:
    if not graph_id:
        return False
    try:
        LocalGraphStore.instance().load_graph(graph_id)
        return True
    except FileNotFoundError:
        return False
    except Exception as exc:
        logger.warning(f"检查本地图谱存在性失败: graph_id={graph_id}, error={exc}")
        return False


def _rebuild_local_graph_for_project(project) -> str:
    previous_graph_id = str(getattr(project, "graph_id", "") or "")
    text = ProjectManager.get_extracted_text(project.project_id) or ""
    ontology = project.ontology or {}
    builder = GraphBuilderService()
    graph_id = builder.create_graph(project.name or "MiroFish Local Graph")
    builder.set_ontology(graph_id, ontology)
    if text.strip():
        chunks = TextProcessor.split_text(
            text,
            project.chunk_size or Config.DEFAULT_CHUNK_SIZE,
            project.chunk_overlap or Config.DEFAULT_CHUNK_OVERLAP,
        )
        if chunks:
            builder.add_text_batches(graph_id, chunks, batch_size=3)
    project.graph_id = graph_id
    project.graph_build_task_id = f"local-rebuild-{uuid.uuid4().hex[:8]}"
    project.status = ProjectStatus.GRAPH_COMPLETED
    project.error = None
    ProjectManager.save_project(project)
    logger.warning(
        f"检测到项目 {project.project_id} 的本地图谱缺失，已自动重建: old={previous_graph_id or '-'}, new={graph_id}"
    )
    return graph_id


def _ensure_simulation_graph_ready(manager: SimulationManager, state, project):
    if not Config.use_local_graph_backend():
        return state.graph_id, False

    preferred_ids = []
    for candidate in [state.graph_id, getattr(project, "graph_id", None)]:
        candidate = str(candidate or "").strip()
        if candidate and candidate not in preferred_ids:
            preferred_ids.append(candidate)

    for candidate in preferred_ids:
        if _local_graph_exists(candidate):
            if candidate != state.graph_id:
                state.graph_id = candidate
                manager._save_simulation_state(state)
            if candidate != getattr(project, "graph_id", None):
                project.graph_id = candidate
                ProjectManager.save_project(project)
            return candidate, False

    graph_id = _rebuild_local_graph_for_project(project)
...
def prepare_simulation():
    ...
    try:
        ...
        healed_graph_id, healed = _ensure_simulation_graph_ready(manager, state, project)
        if healed:
            logger.info(f"prepare 请求已自动修复缺失图谱: simulation_id={simulation_id}, graph_id={healed_graph_id}")
...
def start_simulation():
    ...
    try:
        ...
        healed_graph_id, healed = _ensure_simulation_graph_ready(manager, state, project)
        if healed:
            logger.info(f"start 请求已自动修复缺失图谱: simulation_id={simulation_id}, graph_id={healed_graph_id}")
